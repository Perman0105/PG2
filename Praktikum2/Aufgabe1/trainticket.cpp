...
string TrainTicket::showDetails()
{
        ...
        {
            return "-Zugticket von "+getFromDestination()+" nach "+getToDestination()+" am "+getFromDate()+
            " um "+getDepartureTime()+" über nichts. Preis: "+to_string(this->price)+ " Euro";
        }
}
...
string TrainTicket::getType()
{
    return "T";
}

int TrainTicket::connectingStationsAnzahl()
{
    return connectingStations.size();
}


string TrainTicket::getConnectingStation(int pos)
{
    return connectingStations[pos];
}
